    ...
    def action_checklist_tracking(self):
        ...
        if self.choose_step == 'next_worklow_step' and not self.is_next_workflow and self.lead_id.workflow_master_id:
            ...
            line_vals = [(0, 0, {
                ...
                'sequence': len(self.lead_id.checklist_tracking_line) + 1,
                'comments': self.comments,
                'landoc_stage': self.landoc_stage,
                'department_id': self.department_id.id,
                'stage_id': self.stage_id.id,
                'active_step': True,
                'workflow_line_id': self.workflow_line_id.id
            })]
            if self.lead_id:
                for rec in self.lead_id.checklist_tracking_line:
                    rec.active_step = False
                self.lead_id.checklist_tracking_line = line_vals
                self.lead_id.team_id = self.department_id.id
                ...
                <ul style="list-style:none; padding-left:0;">
                    <li>
                        <span style="font-weight:bold; color:#6c757d;">{previous_landoc_stage}</span>
                        <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                        <span style="font-weight:bold; color:#0180A5;"> {self.landoc_stage} </span>(Landoc Stage)
                    </li>
                    <li>
                        <span style="font-weight:bold; color:#6c757d;">{previous_stage}</span>
                        <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                        <span style="font-weight:bold; color:#0180A5;"> {self.stage_id.name} </span> (Stage)
                    </li>
                </ul>
                """
                text = Markup(new_string)
                self.lead_id.message_post(body=text)
        ...
        if self.crm_checklist_tracking_id and self.choose_step == 'jump_next_checklist_step':
            for rec in self.lead_id.checklist_tracking_line:
                ...
                if rec.id == self.crm_checklist_tracking_id.id:
                    ...
                    self.lead_id.team_id = rec.department_id.id
